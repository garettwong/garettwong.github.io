...
import {attachHazardCollisions} from './power-core/hazards.js';
...
export async function startPowerPlayer(game,send){
 ...
 let binary='';const bytes=new Uint8Array(game.bytes);for(let i=0;i<bytes.length;i+=8192)binary+=String.fromCharCode(...bytes.subarray(i,i+8192));nes.loadROM(binary);attachPowerTiming(nes,game.id===POWER_ROM?8:12);attachHazardCollisions(nes);
