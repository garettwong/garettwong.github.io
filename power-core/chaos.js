...
const MISSILE_COLORS={3:0xff268aff,8:0xffb3ff86,9:0xffffd780,10:0xff80cfff,11:0xfffff0b0,12:0xffffa8ed,13:0xff80cfff,14:0xffffffff};
...
export function createChaos(nes,profile,density='more',directions=64){
 if(profile==='WAVE')profile='S';
 directions=[16,32,64,128,256,512,1024].includes(directions)?directions:64;
 ...
 function addShot(k,p,cx,cy,a){if(free.length<=(k===14?0:32)){blocked++;return false;}const i=free.pop();on[i]=1;kind[i]=k;owner[i]=p;age[i]=0;angle[i]=a;dirCos[i]=Math.cos(a);dirSin[i]=Math.sin(a);ox[i]=x[i]=px[i]=cx;oy[i]=y[i]=py[i]=cy;active++;emitted++;return true;}
 ...
 function emit(p,m){
  const kinds=profile==='SFL'?[0,1,2]:[{S:0,F:1,L:2,COMET:3,WAVE:4,STAR:5,SWARM:8,SKYFALL:9,PACK:10,LANCE:11,HELIX:12,FRONT:8}[profile]??0],needed=directions*kinds.length;
  if(free.length<needed+32){blocked++;return false;} // Never erase a live ring to admit another.
 ...
 function before(){
  ...
  for(let p=0;p<2;p++){
   ...
   if(profile==='FRONT'&&down&&frame%3===0){const aim=m[0x40]?-Math.PI/2:([-Math.PI/2,-Math.PI/4,0,Math.PI/4,0,Math.PI,Math.PI*.75,Math.PI,-Math.PI*.75,-Math.PI/2,Math.PI/2][m[0xc2+p]]??(m[0xd8+p]&64?Math.PI:0));addShot(14,p,m[0x334+p],m[0x31a+p]-8,aim);}
